...
def send_ai_subtask_reminders(subtask_id=None, user_id=None):
    ...
    if subtask_id:
        try:
            ai_subtask = AiSubTask.objects.get(id=subtask_id, status__in=['pending', 'in-progress'])
        ...
        ai_subtasks = AiSubTask.objects.filter(status__in=['pending', 'in-progress'], user_id=user_id)
        logger.info(f"Sending reminders for user_id: {user_id}")
    ...
    for ai_subtask in ai_subtasks:
        try:
            ...
            reminder_utc = reminder_local.astimezone(pytz.UTC)
            logger.debug(f"Reminder time (UTC) for ai subtask '{ai_subtask.title}': {reminder_utc}, current time: {current_time}")

            if reminder_utc <= current_time <= reminder_utc + timedelta(minutes=5):
